...
const useAuthStore = create((set, get) => ({
  ...
  fetchUserProfile: async (userId) => {
    try {
      ...
      // Actualizar userProfile con el rol actual del estado si existe
      const currentUser = get().user;
      const currentProfile = get().userProfile;

      if (currentUser && currentProfile) {
        set({ userProfile: { ...usuario, rol: currentProfile.rol } });
      }
    } catch (error) {
      ...
      if (error.message && (error.message.includes('404') || error.message.includes('no encontrado'))) {
        ...
        if (currentProfile) {
          set({
            cliente: null,
            userProfile: { rol: currentProfile.rol }
          });
        }
      } else {
      ...
      if (token) {
        try {
          ...
          if (decoded.exp && decoded.exp < currentTime) {
            ...
            // localStorage.removeItem('usuario'); // Ya no se usa
            set({
              user: null,
              isAuthenticated: false,
              loading: false
          ...
          set({
            user: { id: userData.id, email: userData.correo },
            userProfile: userData,
            isAuthenticated: true,
            isAdmin: userData.rol === 'super_admin',
            loading: false
          });

          // Intentar obtener perfil del cliente
          await get().fetchUserProfile(userData.id);
        } catch (error) {
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      // localStorage.setItem('usuario', JSON.stringify(userData)); // Eliminado
  ...
  logout: async () => {
    ...
    try {
      ...
      localStorage.removeItem('token');
      localStorage.removeItem('usuario'); // Limpiar por si acaso existía de antes
